...
public class FriendsContainer extends Container {
    ...
    private void init() {
        ...
        if (ServerAPI.me()!= null )
                friendCount = ServerAPI.me().friendRequests.size();
        ...
        if(friendCount == 0) {
            ...
            for(User u : ServerAPI.me().friendRequests) {
                ...
                if(u.avatar.get() != null) {
                    i = URLImage.createCachedImage(u._id.get()+"-avatar.jpg", 
    ...
    private void bindAddRemoveFriendEvent(
        ...
        add.addActionListener(e -> {
            ServerAPI.sendFriendRequest(u._id.get());
    ...
    private void bindConfirmDeleteEvent(
        ...
        add.addActionListener(e -> {
            findParent(remove.getParent()).remove();
            animateLayout(150);
            if(ServerAPI.acceptFriendRequest(u._id.get())) {
                ServerAPI.refreshMe();
