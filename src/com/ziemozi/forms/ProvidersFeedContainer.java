...
public class ProvidersFeedContainer extends InfiniteContainer {
        ...
        if(index == 0) {
            ...
            response.add(UIUtils.createSpace());                
                ...
                providers = localAPI.searchProviders(
                        "", 0, amount + 9999);
            ...
            if ( index + amount > providers.size()) {
                amount = providers.size() - index;
                if ( amount <=0) return null;
            }

            for (int iter =0; iter < amount; iter++) { 
             int offset = index+iter;
             response.add(createEntry(providers.get(offset)));
            }
            if (response.isEmpty()) {
                return null;
            }
    ...
    private Component createEntry(Provider p) {
       ...
       private static Container createWelcomeBar() {
        ...
        String w=" ****Pull To Refresh List of Proviers.*** ";
        welcome.setText(w);
        welcome.setEditable(false);
        //welcome.setUIID("SmallBlueLabel");
        Button map = new Button("Pull To Refresh List Of Providers");
