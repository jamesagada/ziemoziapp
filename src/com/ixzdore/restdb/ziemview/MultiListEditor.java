 ...
 * calculated
 */
public class MultiListEditor extends BaseEditorImpl {
    ...
    public final CheckBoxList mlist = new CheckBoxList(new DefaultListModel());

    public MultiListEditor() {
        ...
        mlist.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                String s = "";
                int[] selected = mlist.getMultiListModel().getSelectedIndices();
                for (int i : selected) {
                    if (s.length() > 1) {
                        s = mlist.getMultiListModel().getItemAt(i).toString() + "," + s;
                    }
                    if (s.length() < 1) {
                        s = mlist.getMultiListModel().getItemAt(i).toString();
                    }
                }
    ...
    public void setUpOptions() {
        ...
        //String[] parameterOptions = getOptionsFromParameter();
                    StringBuilder s = new StringBuilder();
                    s.append(serviceAttribute.default_value.get());
        for (int i = 0; i < opa.length; i++) {
            ...
            //mlist.getModel().addItem(optionArray[i]);
            mlist.getMultiListModel().addItem(optionArray[i]);
        }
            //check and mark the selected items        
        for (int i = 0; i < opa.length; i++){
            if (serviceAttribute.default_value.get().
                    contains(mlist.getMultiListModel().getItemAt(i).toString())) {
                //the default value is in the selected, so we mark it as selected
                mlist.getMultiListModel().addSelectedIndices(i);
            }
        }
    }

    private String[] getOptionsFromParameter() {
        Object[] opa = StringUtil.tokenize(requestParameter.value.get(), ",").toArray();
        String[] pArray = new String[opa.length];
        for (int i = 0; i < opa.length; i++) {
            pArray[i] = (String) opa[i];
        }
        return pArray;
    }
}
